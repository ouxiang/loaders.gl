class Tileset {
  ...
  selectTiles(frameState) {
    ...
    var root = this.root;
    updateTile(this, root, frameState);
    ...
    if (!skipLevelOfDetail(this)) {
      this.executeBaseTraversal(this, root, frameState);
    } else if (this.immediatelyLoadDesiredLevelOfDetail) {
      this.executeSkipTraversal(this, root, frameState);
    } else {
      this.executeBaseAndSkipTraversal(this, root, frameState);
    }
  ...
  executeBaseTraversal(this, root, frameState) {
    var baseScreenSpaceError = this._maximumScreenSpaceError;
    var maximumScreenSpaceError = this._maximumScreenSpaceError;
    this.executeTraversal(this, root, baseScreenSpaceError, maximumScreenSpaceError, frameState);
  }

  executeSkipTraversal(this, root, frameState) {
  ...
  executeBaseAndSkipTraversal(this, root, frameState) {
    var baseScreenSpaceError = Math.max(this.baseScreenSpaceError, this.maximumScreenSpaceError);
    var maximumScreenSpaceError = this.maximumScreenSpaceError;
    executeTraversal(this, root, baseScreenSpaceError, maximumScreenSpaceError, frameState);
    traverseAndSelect(this, root, frameState);
  }

  skipLevelOfDetail(this) {
    return this._skipLevelOfDetail;
  }

  addEmptyTile(this, tile) {
    this._emptyTiles.push(tile);
  }

  selectTile(this, tile, frameState) {
  ...
  selectDescendants(this, root, frameState) {
      ...
      for (var i = 0; i < childrenLength; ++i) {
        ...
        if (isVisible(child)) {
          if (child.contentAvailable) {
            updateTile(this, child, frameState);
            touchTile(this, child, frameState);
            selectTile(this, child, frameState);
  ...
  selectDesiredTile(this, tile, frameState) {
    if (!skipLevelOfDetail(this)) {
    ...
    if (defined(loadedTile)) {
      ...
      selectDescendants(this, tile, frameState);
    }
  }

  visitTile(this, tile, frameState) {
    ++this._statistics.visited;
    tile._visitedFrame = frameState.frameNumber;
  }
}

function touchTile(this, tile, frameState) {
...
function getPriority(this, tile) {
  ...
  // A this that has both additive and replacement tiles may not prioritize tiles as effectively since SSE and distance
  // are different types of values. Maybe all priorities need to be normalized to 0-1 range.
  if (tile.refine === Cesium3DTileRefine.ADD) {
    return tile._distanceToCamera;
  }
  var parent = tile.parent;
  var useParentScreenSpaceError = defined(parent) && (!skipLevelOfDetail(this) || (tile._screenSpaceError === 0.0) || parent.hasTilesetContent);
...
function loadTile(this, tile, frameState) {
  if (hasUnloadedContent(tile) || tile.contentExpired) {
    tile._requestedFrame = frameState.frameNumber;
    tile._priority = getPriority(this, tile);
    this._requestedTiles.push(tile);
  }
}

function updateVisibility(this, tile, frameState) {
...
function anyChildrenVisible(this, tile, frameState) {
  var anyVisible = false;
  var children = tile.children;
  var length = children.length;
  for (var i = 0; i < length; ++i) {
    var child = children[i];
    updateVisibility(this, child, frameState);
    anyVisible = anyVisible || isVisible(child);
  }
  return anyVisible;
}

function meetsScreenSpaceErrorEarly(this, tile, frameState) {
  var parent = tile.parent;
...
function updateTileVisibility(this, tile, frameState) {
  updateVisibility(this, tile, frameState);
  ...
  if (tile.hasTilesetContent && hasChildren) {
    ...
    updateTileVisibility(this, child, frameState);
    tile._visible = child._visible;
    return;
  }

  if (meetsScreenSpaceErrorEarly(this, tile, frameState)) {
    tile._visible = false;
    return;
  }
  ...
  if (replace && useOptimization && hasChildren) {
    if (!anyChildrenVisible(this, tile, frameState)) {
...
function updateTile(this, tile, frameState) {
  updateTileVisibility(this, tile, frameState);
...
function reachedSkippingThreshold(this, tile) {
...
function updateAndPushChildren(this, tile, stack, frameState) {
  var i;
  var replace = tile.refine === Cesium3DTileRefine.REPLACE;
  var children = tile.children;
  var length = children.length;
  ...
  for (i = 0; i < length; ++i) {
    updateTile(this, children[i], frameState);
  }
  ...
  var checkRefines = !skipLevelOfDetail(this) && replace && !hasEmptyContent(tile);
  var refines = true;
  ...
  for (i = 0; i < length; ++i) {
    ...
    if (isVisible(child)) {
      ...
      loadTile(this, child, frameState);
      touchTile(this, child, frameState);
    }
    if (checkRefines) {
      var childRefines;
      if (!child._inRequestVolume) {
        ...
        childRefines = executeEmptyTraversal(this, child, frameState);
      } else {
...
function inBaseTraversal(this, tile, baseScreenSpaceError) {
  if (!skipLevelOfDetail(this)) {
    return true;
  }
...
function canTraverse(this, tile) {
...
function executeTraversal(this, root, baseScreenSpaceError, maximumScreenSpaceError, frameState) {
  ...
  while (stack.length > 0) {
    ...
    var tile = stack.pop();
    var baseTraversal = inBaseTraversal(this, tile, baseScreenSpaceError);
    ...
    if (canTraverse(this, tile)) {
      refines = updateAndPushChildren(this, tile, stack, frameState) && parentRefines;
    }
    ...
    if (hasEmptyContent(tile)) {
      ...
      addEmptyTile(this, tile, frameState);
      loadTile(this, tile, frameState);
      if (stoppedRefining) {
        selectDesiredTile(this, tile, frameState);
      }
    } else if (add) {
      ...
      if (baseTraversal) {
        ...
        selectDesiredTile(this, tile, frameState);
        loadTile(this, tile, frameState);
      } else if (reachedSkippingThreshold(this, tile)) {
    ...
    visitTile(this, tile, frameState);
    touchTile(this, tile, frameState);
    tile._refines = refines;
  }
}

function executeEmptyTraversal(this, root, frameState) {
  ...
  while (stack.length > 0) {
    ...
    var traverse = hasEmptyContent(tile) && canTraverse(this, tile);
    ...
    updateTile(this, tile, frameState);
    if (!isVisible(tile)) {
 ...
 */
function traverseAndSelect(this, root, frameState) {
  ...
  while (stack.length > 0 || ancestorStack.length > 0) {
    ...
    if (ancestorStack.length > 0) {
      ...
      if (waitingTile._stackLength === stack.length) {
        ...
        selectTile(this, waitingTile, frameState);
        continue;
      }
    }
